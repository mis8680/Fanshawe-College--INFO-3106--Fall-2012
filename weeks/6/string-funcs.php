...
/*
 * Finding the postion of a token/neddle (piece of string) in a string 
 */
$pos_of_everyone = strpos($str,'Everyone');

//It is important to note that strpos() returns the position -1. Therefore a string's fitst charcter is postion 0.
...
/*
 * Example as to why it is important to check this return with === not ==
 */

$check_with_identical = strpos($str, 'Hi');
...
/*
 * Substring ... select/ retrieve portions of a string based on start position and length values
 */
 ...
 * Thoughts: Check to ensure the strlen is >= 4. Ensure is is even, if not handle this in the way that you want: Perhaps middle -1 vs. middle -2
 */
...
if(strlen($str)%2 == 0)
{
    print substr($str,(strlen($str)/2)-2, 4) . '<br />';
}
else 
{
    print substr($str,(strlen($str)/2)-1, 4) . '<br />';
}


print 'Replace "Everyone" to "People" = ' . substr_replace($str, "People", strpos($str, 'Everyone'), -4) . '<br />'; 
...
print strtr('Hello world, @this_class', '@this_class', 'INFO-3106') . '<br />';

//using [] (square brackets) with strings...
print 'Very fitst character in $str is = ' . $str[0] . '<br />';
